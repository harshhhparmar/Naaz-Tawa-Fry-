        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
          {THAAL_SPECIALS.map((thaal, index) => (
            <motion.div 
              ...
              className={`rounded-2xl p-8 border ${index === 1 ? 'bg-red-950/40 border-red-500/30' : 'bg-white/5 border-white/10'} backdrop-blur-sm relative overflow-hidden group hover:border-red-500/50 transition-colors`}
            >
              {index === 1 && (
                <div className="absolute top-0 right-0 bg-red-600 text-white text-xs font-bold px-3 py-1 uppercase tracking-wider rounded-bl-lg">
                  Most Popular
                </div>
              )}
              ...
              <button className={`w-full mt-8 py-4 rounded-xl font-bold tracking-wide transition-all ${
                index === 1 
